...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteRow(row) {
                Sequelize.WhiteList.destroy({where: {id: row.id}}).then(() => this.search()).catch(err => {
                    throw err
                })
            },
            dialogCheck() {
                if (this.dialogForm.sku || this.dialogForm.name) {
                    if (this.dialogForm.id) {
                        ...
                        Sequelize.WhiteList.create(this.dialogForm).then(() => {
                            this.dialogForm = {}
                            this.dialogFormVisible = false
                            this.search()
                        }).catch(err => {
            ...
            download() {
                this.$electron.remote.dialog.showSaveDialog({
                    ...
                    if (filename) {
                        Sequelize.WhiteList.findAll({where: parseWhiteList(this.whiteListBak)}).then(whiteLists => fs.writeFileSync(filename, convertWhiteListsToExcelBuffer(whiteLists))).catch(err => {
                            throw err
                        })
                    }
                })
            ...
            search() {
                ...
                Sequelize.WhiteList.count({where: parseWhiteList(this.whiteList)}).then(count => this.total = count).catch(err => {
                    throw err
                })
                Sequelize.WhiteList.findAll({
                    limit: this.pageSize,
